...
t_dur = 0:dt:2;
inner_step = 25;
% initial_state = Twelve_Traj(1,[2:13]);
initial_state = 0.01*ones(12,1);
dimension = 12;

Initial_P = eye(12);
Initial_l = (0.01)^2;

delta_initial_up = zeros(12,1);
delta_initial_up(1) = sqrt(Initial_l);
%Lipschitz constant of te system, user input
Lip_constant = 0;
[t_precise,x_precise] = ode45(@Twelve,t_dur,initial_state);
...
%         else
        gamma = shrink;
        Delta = abs(Twelve_Jac(x_relative_min)-Twelve_Jac(x_relative_max));
...
%             [gamma,Q] = QLF(A_c);
        delta_theta = (abs(x(i+1,:)-x(i,:))/2+sqrt(max(eig(inv(M/level)))))*exp(Lip*(t(i+1)-t(i)));
        Delta_matrix = abs(Twelve_Jac(Average_value+delta_theta)-Twelve_Jac(Average_value-delta_theta))/2;
        %disp((Delta_matrix));
...
%    end
    %     disp('eigenvalue of P');disp(eig(Q));
        P_level_original = min_level_set12(Q,M,level);
    end
